    ...
    public function onMessage($connection, $buffer)
    {
        ...
        switch($cmd) 
        {
            ...
            case 'insert':
                ...
                break;
            case 'pop':
				$arr = isset($this->_dataArray[$key]) ? array_shift($this->_dataArray[$key]) : null;
                $connection->send(serialize($arr));
                break;
            case 'list':
				$list = isset($this->_dataArray[$key]) && count($this->_dataArray[$key])>0 ? $this->_dataArray[$key] : null;
                $connection->send(serialize($list));
                break;
            default:
